...
/* Hero Section Styles */
.hero {
    background: url('https://images.unsplash.com/photo-1504674900247-0877df9cc836?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80') no-repeat center center/cover;
    color: #fff;
    text-align: center;
    padding: 5rem 0;
}
...
.search-sort-container input[type="text"]:focus {
    border-color: #666; /* Darken border on focus */
...
.menu-item button:hover {
    background-color: #4cae4c; /* Darken background on hover */
}
...
function updateCartDisplay() {
    ...
    cart.forEach(item => {
        ...
        total += parseFloat(item.price.replace('$', '')); // Remove '$' and convert to number
    });
...
function updateCartDisplay() {
    ...
    // Loop through the cart and add items to the display
    cart.forEach((item, index) => { // Add index to the loop
        const listItem = document.createElement('li');
        listItem.innerHTML = `
            <span>${item.title}</span>
            <span>${item.price}</span>
